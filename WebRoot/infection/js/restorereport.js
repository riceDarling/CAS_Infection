$(function() {
	...
	$("#restorereport_from_addbtn").click(function() {
		if(AllisNes()){
			var data=$("#restorereport_from").serialize()+"&restore=1&&cardId=" + GetRequest();
			$.ajax({
				url : config.url + "report/restore",
				data : data,
				type : "post",
				timeout : 10000,
		...
		success : function(data, status) {
			if (data.error != null) {
				...
				$("<img src='http://" + data.t + "' class= '" + data.t + "' title = '" + data.t + "' " + "class='" + data.t + "' style='border: 1px solid #DDD9D0;cursor:pointer;margin-right:5px;width:16%;'>").appendTo($('.vip_img')).click(function() {
					...
					$("input[name='restoreImg']").val($("input[name='restoreImg']").val().replace(class_name + ",", ""));
					pic_num = pic_num - 1;

				});
				$("input[name='restoreImg']").val($("input[name='restoreImg']").val() + data.t + ",");
			}
		},
	...
	$.ajax({
		...
		success : function(data) {
			if (data.success) {
				...
				if (data.t.restoreImg == null) {
					//$(".edittable tbody tr:eq(21) td:eq(1)").text("");
				} else {
					// $(".edittable tbody tr:eq(21) td:eq(1)
					// input[name='diagnosisImg']").attr("value",data.t.diagnosisImg);
					$("input[name='restoreImg']").val(data.t.diagnosisImg);
					// 定义一数组
					var strs = new Array();
					// 字符分割
					strs = data.t.restoreImg.split(",");
					var html = "";
				...
				$(".btn").click(function() {
					...
					$("input[name='restoreImg']").val($("input[name='restoreImg']").val().replace(title + ",", ""));
					pic_num = pic_num - 1;
				});
