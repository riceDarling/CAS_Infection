$(function() {
	...
	$("#implementation_from_addbtn").click(function() {
		if(AllisNes()){
			var data=$("#implementation_from").serialize()+"&cardId=" + GetRequest();
			$.ajax({
		...
		success : function(data, status) {
			if (data.error != null) {
				...
				$("<img src='http://" + data.t + "' class= '" + data.t + "' title = '" + data.t + "' " + "class='" + data.t + "' style='border: 1px solid #DDD9D0;cursor:pointer;margin-right:5px;width:16%;'>").appendTo($('.vip_img')).click(function() {
					...
					$("input[name='implementationimg']").val($("input[name='implementationimg']").val().replace(class_name + ",", ""));
					pic_num = pic_num - 1;

				});
				$("input[name='implementationimg']").val($("input[name='implementationimg']").val() + data.t + ",");
			}
		},
	...
	$.ajax({
		...
		success : function(data) {
			if (data.success) {
				...
				if (data.t.implementationimg == null) {
					//$(".edittable tbody tr:eq(21) td:eq(1)").text("");
				} else {
					// $(".edittable tbody tr:eq(21) td:eq(1)
					// input[name='diagnosisImg']").attr("value",data.t.diagnosisImg);
					$("input[name='implementationimg']").val(data.t.implementationimg);
					// 定义一数组
					var strs = new Array();
					// 字符分割
					strs = data.t.implementationimg.split(",");
					var html = "";
				...
				$(".btn").click(function() {
					...
					$("input[name='implementationimg']").val($("input[name='implementationimg']").val().replace(title + ",", ""));
					pic_num = pic_num - 1;
				});
