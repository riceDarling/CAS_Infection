$(document).ready(function() {
	...
	$("#age_select_btn").click(function() {
		
		console.log($("#age_select_form").serialize());
		getStatisticsByAge($("#age_select_form").serialize());
	});
});
function getStatisticsByAge(data_json) {
	getPageContent("statisticsByAge", "/Infection/statistical/getStatisticsByAge", 1, $(".YApage>td"), data_json);
};
function pushListContent(data) {
	var html = "";
//	for (var i = 0; i < data.t.list[0].length; i++) {
//		html += "<tr>";
//		html += "<td>" + data.t.list[0][i].age + "岁</td>";
...
//	}
	for (var i = 0; i < data.t.list.length; i++) {
		html += "<tr>";
		html += "<td>" + data.t.list[i].age + "岁</td>";
