$(function() {
	...
	$("#trackingreport_from_addbtn").click(function() {
		if(AllisNes()){
			var data=$("#trackingreport_from").serialize()+"&cardid=" + GetRequest();
			$.ajax({
				url : config.url + "report/insertTracking",
				data : data,
				type : "post",
				timeout : 10000,
		...
		success : function(data, status) {
			if (data.error != null) {
				...
				$("<img src='http://" + data.t + "' class= '" + data.t + "' title = '" + data.t + "' " + "class='" + data.t + "' style='border: 1px solid #DDD9D0;cursor:pointer;margin-right:5px;width:16%;'>").appendTo($('.vip_img')).click(function() {
					...
					$("input[name='trackingimg']").val($("input[name='trackingimg']").val().replace(class_name + ",", ""));
					pic_num = pic_num - 1;

				});
				$("input[name='trackingimg']").val($("input[name='trackingimg']").val() + data.t + ",");
			}
		},
